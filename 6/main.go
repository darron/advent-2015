...
func main() {
	// Set the whole grid to 0.
	setupGrid(xMin, yMin, xMax, yMax)

	// For each line - parse and figure out:
	// 1. The operation requested.
	// 2. First co-ordinates.
	// 3. Second co-ordinates.
	// line1 := "turn off 660,55 through 986,197"
	// parseLine(line1)
	//
	// line2 := "turn on 226,196 through 599,390"
	// parseLine(line2)
	//
	// line3 := "toggle 322,558 through 977,958"
	// parseLine(line3)

	ordersSlice := strings.Split(orders, "\n")

	for _, line := range ordersSlice {
		parseLine(line)
	}
...
func getCoordinates(line string, first int, second int) (int, int, int, int) {
	pieces := strings.Split(line, " ")
	firstSet := strings.Split(pieces[first], ",")
	secondSet := strings.Split(pieces[second], ",")
	x1, _ := strconv.ParseInt(firstSet[0], 10, 64)
	y1, _ := strconv.ParseInt(firstSet[1], 10, 64)
	x2, _ := strconv.ParseInt(secondSet[0], 10, 64)
	y2, _ := strconv.ParseInt(secondSet[1], 10, 64)
...
func toggle(line string) {
	x1, y1, x2, y2 := getCoordinates(line, 1, 3)
	fmt.Printf("x1: %d, y1: %d, x2: %d, x2: %d\n", x1, y1, x2, y2)
	flip(x1, y1, x2, y2)
}

func flip(x1, y1, x2, y2 int) {
	for y := y1; y <= y2; y++ {
		for x := x1; x <= x2; x++ {
			value := grid[x][y]
			if value == 0 {
...
func setupGrid(xMin, yMin, xMax, yMax int) {
	for y := yMin; y <= yMax; y++ {
		for x := xMin; x <= xMax; x++ {
			grid[x][y] = 0
		}
